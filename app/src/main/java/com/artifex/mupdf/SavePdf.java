...
public class SavePdf {
    ...
    float widthScale;       //不知道是屏幕的宽高还是应该是pdf的宽高
    float heightScale;
    ...
    public void addText() {
        try {
            ...
            img.setAlignment(1);
            //这里是重点！原始363*557！设置Image图片大小，需要根据屏幕的分辨率，签名时PDF的放大比例来计算；还有就是当PDF开始显示的时候，他已经做了一次缩放，可以用 rectangle.getWidth() / (bitmap.getWidth() / 2)求得那个放大比
            img.scaleAbsolute(353 * 1.0f * density / 2 / scale * rectangle.getWidth() / (bitmap.getWidth() / 2), 550 * 1.0f * density / 2 / scale * rectangle.getWidth() / (bitmap.getWidth() / 2));
